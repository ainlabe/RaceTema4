...
```{r}
avg_bytes <- cran %>% summarise(avg_bytes = mean(size, na.rm = TRUE)) %>% pull(avg_bytes)
...
top_unique_sorted <- cran %>%
  group_by(package) %>%
  summarise(unique = n_distinct(ip_id)) %>%  
...
cran %>%
  ...
  ggplot(aes(x = reorder(country, -total_traffic), y = total_traffic)) +
  geom_point(color = "blue", size = 3) +
  labs(
    title = "Países con mayor tráfico de información",
    x = "Código de País",
    y = "Tráfico Generado (Bytes)"
  ) +
  theme_minimal()
...
students2_tidy <- students2 %>%
  pivot_longer(
    cols = c(male_1, female_1, male_2, female_2), 
...
all_students <- bind_rows(failed, passed)
all_students
```
...
```{r}
ggplot(sat_tidy, aes(x = score_range, y = prop, color = sex)) +
  geom_line(size = 1) +                     
  geom_point(size = 2) +                     
  facet_wrap(~ part, ncol = 1) +              
  labs(
    title = "Diferencias entre hombres y mujeres en las pruebas SAT",
    x = "Franja de Calificación (Score Range)",
    y = "Proporción",
    color = "Sexo"
  ) +
  theme_minimal() +                            
